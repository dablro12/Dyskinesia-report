...
def ml_classifier(ckpt_path, data_path, windows=None):
    ...
    if windows is not None:
        ...
        if config['c_out'] == 1:
            ...
            X_infer, _ = reshape_multi_tensor(infer_loader)
            
        model, config = init_model(X_infer, _, config, bayes_opt= False)
        preds = model.inference_forward(reshape_X = X_infer, weight = weight, thr = best_thr) # (All Frame - Window Size) / Stride + 1
